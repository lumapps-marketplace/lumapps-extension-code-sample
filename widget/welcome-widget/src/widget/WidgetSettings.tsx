...
type WidgetSettingsProps = import("lumapps-sdk-js").SettingsComponent<
  import("./types").SampleAppGlobalParams,
  import("./types").SampleAppParams
>;

const WithIntlSettings: WidgetSettingsProps = ({ properties = {}, exportProp }) => {
  const intl = useIntl();
  const FONT_SIZES = [
    {
      label: "16px",
  ...
  const toggleSelectComaPosition = () =>
    setOpenComaPosition(!isOpenComaPosition);
...
export const WidgetSettings: WidgetSettingsProps = (props) => {
  const { displayLanguage } = useLanguage();
  const messages: Record<string, Record<string, string>> = React.useMemo(() => ({
    en: messagesEn,
    fr: messagesFr,
    de: messagesDe,
    es: messagesEs,
